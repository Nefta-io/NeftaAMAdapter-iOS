...
class RewardedSim : UIView {
    ...
    public class Track : NSObject, @unchecked Sendable, GADFullScreenContentDelegate {
        ...
        func OnLoad(rewarded: GADRewardedAd) {
            ...
            _rewarded!._paidEventHandler = onPaid
        ...
        func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
            RewardedSim.Instance.Log("adDidDismissFullScreenContent")
    ...
    private func GetInsightsAndLoad(track: Track) {
        ...
        NeftaPlugin._instance!.GetInsights(Insights.Rewarded, previousInsight: track._insight, callback: { insights in
            ...
            if let insight = insights._rewarded {
                ...
                Task {
                    do {
                        let rewarded = try await SGADRewardedAd.load(withAdUnitID: track._adUnitId, request: track._request)
    ...
    func SimOnAdLoadedEvent(request: Track, isHigh: Bool) {
        if let rewarded = request._rewarded, rewarded._hasFill {
            rewarded._hasFill = false
    ...
    public class SGADRewardedAd : GADRewardedAd {
        ...
        static override func load(withAdUnitID: String, request: GADRequest?) async throws -> SGADRewardedAd {
            var adRequest: Track?
            if await RewardedSim.Instance._trackA._request == request {
                adRequest = await RewardedSim.Instance._trackA
                await RewardedSim.Instance.ToggleTrackA(isOn: true)
                await RewardedSim.Instance.SetStatusA("\(adRequest!._adUnitId) loading")
            } else {
                adRequest = await RewardedSim.Instance._trackB
                await RewardedSim.Instance.ToggleTrackB(isOn: true)
                await RewardedSim.Instance.SetStatusB("\(adRequest!._adUnitId) loading")
            }
            ...
            if adRequest!._loadSelection == 1 {
                let ad = SGADRewardedAd(adUnitId: adRequest!._adUnitId)
                ad._request = request
                ad._hasFill = true
                return ad
            }
